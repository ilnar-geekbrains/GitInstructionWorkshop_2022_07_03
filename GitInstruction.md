...
Найти нужный репозиторий можно с помощью активной ссылки или по логину владельца репозитория. Для этого перейдите по ссылке [Поиск на GitHub](https://github.com/search) и введите известную вам информацию. Найденные данные будут разделены по блокам названия репозиториев, названия комитов, темы, пользователи и прочее.

**ВАЖНО**
Первое действие создание копии удаленного репозитория в своем аккаунте на GitHub. В противном случае, вы не сможете отправить предложенные изменения, так как у вас отсутствуют права на редактирование файлов.

1. Используем кнопку FORK (создали *fork*)

2. Нажимаем зеленую кнопку CODE и копируем ссылку для создания команды в git

3. В VSCode даем используем команду git clone и через пробел вставляем указанную ссылку. 
>Теперь можно вносить изменения.

**ВАЖНО** Перейти через команду cd в клонированный локальный репозиторий, проверить свое положение командой git status. 

4. **ПЕРВОЕ ДЕЙСТВИЕ** - создание новой ветки, в которой и будут происходить все изменения. 

5. В новой ветке создаем файл README с описанием проведенных изменений.

6. После сохранений из новой ветки командой git push отправляем изменения в удаленный репозиторий. Гит может предложить дополнить команду ссылкой, кпируем предложенный вариант и используем его.

7. Со своего аккаунт нажимаем кнопку COMPARE & PULL REQUEST
