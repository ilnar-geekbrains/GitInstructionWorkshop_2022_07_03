...
***Удаленный репозиторий*** - это версии проекта, которые содержат данные на удаленном сервере с определнными правами доступа. Доступ к репозиторию на такоа сервере может осуществляться по интернету или по локальной сети. 
У удаленного репозитория также есть собственные ветки, видны сохранения комитов. 
Просмотр удаленных репозиториев можно проверить командой **git remote**;
Отправка измеений в удаленный репозиторий - **git push <remote-name> <branch-name> **
