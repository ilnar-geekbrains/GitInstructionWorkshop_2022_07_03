...
Система контроля версий это такая программа, которая помогает отслеживать любые изменения сделанные всеми участниками проекта(например среда редактирования VSCode). В частности для локальной работы используется программа git. 
