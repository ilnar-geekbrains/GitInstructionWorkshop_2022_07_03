...
> Репозиторий - папка в которой ведется отслеживание изменений (версионность). Для создания используются команда git init. Затем создаем файлы. Далее добавляем файл для отслеживания git add. После этого можно сделать первый коммит.
