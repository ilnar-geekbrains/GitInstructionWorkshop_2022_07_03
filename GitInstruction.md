...
Репозиторий - это папка или другая директория, где храниться файл, за который ведётся контроль его версий, то есть хроняться все сохраннёные варианты файла. Репозитории могут быт локальными (находятся на вашем компьютере) или удалёнными (находятся на чужом компьютере или на сервере)

Чтобы создать локальный репозиторий на вашем компьютере необходимо создать папку, далее открыть эту папку с помощью программы ***Visual Studia Code*** и далее в терминале набрать команду ***Git Init***. С этого момента за файлами в этой папки будет производиться контроль версий.
