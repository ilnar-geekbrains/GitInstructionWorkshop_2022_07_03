...
Если мы хотим прианести изменения в какой-либо проект, предложить свое решение, мы используем функцию *__pull request__*. Что для этого необходимо сделать?


1. в первую очередь нам необходимо знать название репозитория, с которым хотим работать. Можно воспользоваться ссылкой на пользователя на [github](https://github.com/
).
2. создаем *fork* (копию) этого репозитория к себе в GitHub
3. создаем локальный репозиторий репозиторий командой *git clone ссылка на репозиторий*
4. создаем свою собственную ветку
5. именно в ней работаем, попутно создавая *commit-ы* и *message* 
6. направляем их в свой аккаунт GitHub
7. Жмем **pull-request**
8. Всё!
